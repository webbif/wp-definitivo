# WP Definitivo 1.0.67 accessibility audit
...
- [x] Accessibility remediation is prepared as version `1.0.67` in the working tree.
- [x] Create the version 1.0.64 audit-candidate commit (`848e477`).
- [ ] Create the version 1.0.67 candidate commit after the formal audit passes.
- [ ] Create the final tag after the formal audit is complete.
- [x] `style.css`, `readme.txt`, and `package.json` report version `1.0.67`.
- [x] MyTemplatesWoo runs WordPress 7.1.
- [ ] Synchronize WP Definitivo 1.0.67 to MyTemplatesWoo and verify the final hashes after local audit changes are complete.
...
- [x] Build and validate the current 1.0.67 local candidate after the desktop submenu-indicator correction.
- [ ] Record the final 1.0.67 ZIP contents and hashes.
...
| 2026-09-04 | Desktop submenu indicators | Remediated locally; deployment pending | Version 1.0.67 adds a downward indicator to top-level menu items with children and a direction-aware lateral indicator to nested parents such as Level 2. RTL nested indicators are mirrored. The integrated desktop submenu regression now verifies that both indicator levels are rendered with distinct directions. No deployment was performed. |
| 2026-09-04 | Version 1.0.67 local release checks | Pass | Build, generated translations, JavaScript/CSS/JSON lint, package validation, 68-file inventory, 1200 x 900 screenshot, and asset budgets passed. The candidate remains local and MyTemplatesWoo continues to run 1.0.66. |
...
| 2026-09-04 | Manual zoom: Post with Comments at 400% | Failed, remediated locally; retest pending | The page content reflowed, but the opened header search kept its field, submit button, and close control in one horizontal row, clipping the controls beyond the compact viewport. Version 1.0.67 now stacks the form controls and places the close button on its own line at widths up to 700 CSS pixels. A focused 320px reflow regression was added. No deployment was performed. |
| 2026-09-04 | Manual zoom: comment table at 400% | Failed, remediated locally; retest pending | The unwrapped classic HTML table in a Theme Unit Test comment exceeded its content column and the site's clipping boundary hid the third column. Version 1.0.67 now applies fixed table layout and safe cell wrapping to entry and comment tables at widths up to 700 CSS pixels, preserving native table semantics. A focused 320px regression verifies that the table remains inside the comment and no cell overflows. No deployment was performed. |
| 2026-09-04 | Manual zoom: nested comment metadata at 400% | Failed, remediated locally; retest pending | Comment author rows did not wrap and cumulative nesting indents left deep comments too narrow, causing “disse:” and long author names to touch or cross the card boundary. Version 1.0.67 arranges avatar, author, and “disse:” in a compact grid, safely wraps names, and removes cumulative nested-list indentation at widths up to 700 CSS pixels. A focused 320px regression covers comment depth 10. No deployment was performed. |
