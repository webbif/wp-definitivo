# WP Definitivo 1.0.72 accessibility audit
...
- [x] Accessibility remediation is prepared as version `1.0.72` in the working tree.
...
- [x] Create the version 1.0.71 single-RTL-stylesheet remediation commit (`e4ff602`).
- [x] Create the version 1.0.72 compact-navigation and RTL back-to-top remediation commit (`01d735e`).
- [ ] Create the final tag after the formal audit is complete.
- [x] `style.css`, `readme.txt`, and `package.json` report version `1.0.72`.
...
- [x] Synchronize WP Definitivo 1.0.71 to MyTemplatesWoo and verify all deployed file hashes.
- [x] Synchronize WP Definitivo 1.0.72 to MyTemplatesWoo and verify all deployed file hashes.
...
- [x] Build and validate the current 1.0.72 local candidate after the compact-navigation and RTL back-to-top corrections.
- [x] Record the final 1.0.72 ZIP contents and hashes.
...
| 2026-09-04 | Manual zoom: Post with Comments at 400% | Pass after remediation | The complete page was revalidated after deployment. Header-search controls stack without clipping, comment content and controls remain usable, and the page itself has no horizontal scrolling. |
| 2026-09-04 | Manual zoom: comment table at 400% | Pass after remediation | Version 1.0.68 restores automatic column sizing and whole-word wrapping, contains horizontal scrolling within the table, and makes the table keyboard focusable without replacing its native semantics. The focused regression passed in all five browser profiles, and the real 400% browser-zoom review confirmed readable words, local scrolling, and access to all columns without page-level horizontal scrolling. |
| 2026-09-04 | Manual zoom: nested comment metadata at 400% | Pass after remediation | Comment author rows now wrap within their cards and cumulative nesting indents are removed at compact widths. The author name, “disse:” text, avatar, date, and deeply nested comment content remained readable; the focused regression also passed across all five browser profiles. |
...
| 2026-09-04 | Manual zoom: Category Archive at 200% and 400% | Pass | In Chrome with the established restored-window test setup, the category archive reflowed without page-level horizontal scrolling, clipping, or overlap. The archive title, post cards, images, metadata, excerpts, links, navigation, and header controls remained visible and usable at both zoom levels. |
...
| 2026-09-04 | Manual zoom: 404 page at 200% and 400% | Pass after remediation | Version 1.0.69 adds explicit spacing between the search form and Return home action and stacks the search controls at narrow reflow widths. Real browser-zoom retesting confirmed that the title, message, form, return link, compact navigation, and page layout remain readable and usable at both zoom levels without page-level horizontal scrolling. |
...
| 2026-09-04 | RTL remediation iterations | Pass after second remediation | Version 1.0.70 removed the obsolete physical submenu positioning and converted compact submenu indentation to logical properties. A deployed retest revealed that WordPress also printed an unversioned root `rtl.css`, whose stale cached copy restored the removed rule. Version 1.0.71 now suppresses only that duplicate parent-theme link, retains the versioned parent stylesheet, and preserves a child theme's own locale stylesheet. Two PHPUnit regressions cover the parent and child cases. |
| 2026-09-04 | Version 1.0.71 deployment and integrity | Pass | The canonical theme and active MyTemplatesWoo copy each contain 68 files with zero missing, extra, or SHA-256-mismatched files. The public Arabic page emits `<html dir="rtl" lang="ar">` and loads exactly one `rtl.css?ver=1.0.71`. The previous version is preserved at `/home/u976587618/backups/wp-definitivo-1.0.70-before-1.0.71-e4ff602.tar.gz`. |
| 2026-09-04 | Version 1.0.71 RTL focused matrix | Pass | Reflow on all eight routes, compact menu and accordion behavior, desktop nested submenu positioning and Escape restoration, opened header search, keyboard-accessible table scrolling, and deep comment reflow passed 35/35 across Chromium, Firefox, WebKit, Mobile Chrome, and Mobile Safari. |
| 2026-09-04 | Version 1.0.71 complete RTL matrix | Pass after isolated retry | The complete run produced 201 passes, 8 intentional project-specific skips, and one Firefox timeout while the long unexpected-context test traversed all eight routes. The same scenario passed on isolated retry in 15.8 seconds after receiving a proportional 90-second timeout. No functional assertion failed. Manual visual RTL review remains before the RTL checklist item is closed. |
| 2026-09-04 | Compact submenu Escape order | Failed; remediated in 1.0.72 | Manual RTL testing found that Escape closed the entire compact navigation before the active submenu and preserved expanded submenu state when the menu reopened. Version 1.0.72 now dismisses the most recently active nested submenu first, restores focus to its toggle, closes the main menu only after submenu levels are exhausted, and resets every submenu whenever the main menu closes. A self-contained regression passed in Chromium, Firefox, WebKit, Mobile Chrome, and Mobile Safari. Manual production revalidation remains pending. |
| 2026-09-04 | Manual RTL header search | Pass | At 100% browser zoom, the header search opened without clipping or overlap, moved focus into the search field, exposed every control with visible keyboard focus, closed with Escape, returned focus to the search toggle, and caused no unexpected page movement. |
| 2026-09-04 | Manual RTL comments and back-to-top | Partial pass; remediated in 1.0.72 | Comment layout, alignment, nested content, and focusable links passed manual RTL review. The back-to-top button stretched across the viewport because physical `right` and logical `inset-inline-end` declarations constrained opposite sides in RTL. Version 1.0.72 now uses only logical inline positioning, retaining a compact desktop button and the existing hidden state at widths up to 960px. Manual production revalidation remains pending. |
| 2026-09-04 | Manual RTL table scrolling | Pass | The comment-content table fit without unnecessary scrolling at 100% zoom. At 400%, it received visible keyboard focus and exposed its hidden columns through direction-appropriate internal scrolling without creating horizontal scrolling on the page. |
| 2026-09-04 | Manual RTL comment form | Pass | Labels, fields, required markers, and the submit control aligned and reflowed without clipping or horizontal page scrolling. Forward and reverse keyboard traversal retained visible focus. An empty textarea placed the caret at the inline start on the right, and Arabic input followed the expected RTL text flow. |
| 2026-09-04 | Manual RTL 404 page | Pass | The title, explanatory text, search form, submit control, and return-home action followed the RTL layout without clipping, overlap, or horizontal page scrolling. Keyboard traversal followed the available controls with visible focus. |
| 2026-09-04 | Manual RTL search results | Pass | The search heading, labeled form, submit control, result cards, titles, metadata, excerpts, and links followed the RTL layout without clipping, overlap, or horizontal page scrolling. Keyboard traversal retained visible focus. |
| 2026-09-04 | Manual RTL category archive | Pass | The archive heading, post cards, images, titles, metadata, excerpts, links, and applicable navigation followed the RTL layout without clipping, overlap, or horizontal page scrolling. Keyboard traversal retained visible focus. |
| 2026-09-04 | Manual RTL blog index | Pass | The blog heading, chronological post cards, images, titles, dates, authors, excerpts, links, and applicable pagination followed the RTL layout without clipping, overlap, or horizontal page scrolling. Keyboard traversal retained visible focus. |
| 2026-09-04 | Manual RTL markup and formatting | Pass | Headings, paragraphs, lists, quotations, images, captions, links, tables, and preformatted content followed the RTL layout without clipping, overlap, or horizontal page scrolling. Markers and indentation appeared on the appropriate side, and keyboard focus remained visible. |
| 2026-09-04 | Manual RTL block patterns | Pass | The page title, instructional content, header, compact navigation, links, and footer followed the RTL layout without clipping, overlap, or horizontal page scrolling. Keyboard traversal retained visible focus. No patterns had been inserted into the page content, so the expected instructional placeholder was displayed. |
| 2026-09-04 | Manual RTL front page | Pass | The page content, header, compact navigation, search, links, and footer followed the RTL layout without clipping, overlap, or horizontal page scrolling. Keyboard traversal retained visible focus and the interactive controls behaved as expected. |
| 2026-09-04 | Version 1.0.72 local candidate validation | Pass | Release build and JS/CSS/JSON lint passed for 68 distributable files; the screenshot remained 1200 by 900 pixels; contextual CSS measured 15,134 bytes gzip and navigation JavaScript 1,434 bytes gzip. WordPress and PHP 7.4+ coding/compatibility checks passed on all 38 applicable files, PHPUnit passed 38 tests with 82 assertions, and the two focused browser regressions passed 10/10 across Chromium, Firefox, WebKit, Mobile Chrome, and Mobile Safari. |
| 2026-09-04 | Version 1.0.72 package and deployment | Pass; manual revalidation pending | Commit `01d735e` was pushed to `origin/main`. The canonical ZIP contains exactly 68 files with zero source mismatches and SHA-256 `DC8090D52DE179CC863A9E1ABF069BF7F391437870FFC12DCD9F7B5A84D42CF3`. MyTemplatesWoo runs active version 1.0.72; all 68 deployed files match the canonical source with zero missing, extra, or hash-mismatched files. Public HTML emits `<html dir="rtl" lang="ar">`, `rtl.css?ver=1.0.72`, and `navigation.min.js?ver=1.0.72`. Version 1.0.71 is preserved at `/home/u976587618/backups/wp-definitivo-1.0.71-before-1.0.72-01d735e.tar.gz`. |
