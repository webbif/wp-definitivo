...
- [ ] Create the version 1.0.72 compact-navigation and RTL back-to-top remediation commit.
...
- [ ] Record the final 1.0.72 ZIP contents and hashes.
...
| 2026-09-04 | Compact submenu Escape order | Failed; locally remediated | Manual RTL testing found that Escape closed the entire compact navigation before the active submenu and preserved expanded submenu state when the menu reopened. The local correction now dismisses the most recently active nested submenu first, restores focus to its toggle, closes the main menu only after submenu levels are exhausted, and resets every submenu whenever the main menu closes. A self-contained regression passed in Chromium, Firefox, WebKit, Mobile Chrome, and Mobile Safari. Deployment and manual RTL revalidation remain pending. |
...
| 2026-09-04 | Manual RTL comments and back-to-top | Partial pass; locally remediated | Comment layout, alignment, nested content, and focusable links passed manual RTL review. The back-to-top button stretched across the viewport because physical `right` and logical `inset-inline-end` declarations constrained opposite sides in RTL. The local correction now uses only logical inline positioning, retaining a compact desktop button and the existing hidden state at widths up to 960px. Deployment and manual RTL revalidation remain pending. |
